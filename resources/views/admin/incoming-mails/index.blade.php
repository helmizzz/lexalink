<x-admin-layout>
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('admin.incoming-mails.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-semibold text-sm transition">
            + Tambah Surat Masuk
        </a>
    </div>

    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
        <form method="GET" action="{{ route('admin.incoming-mails.index') }}" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari No Surat, Pengirim..." class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
            </div>
            ...
            <a href="{{ route('admin.incoming-mails.index') }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 font-semibold text-sm transition">
                Reset
            </a>
        </form>
    </div>
                ...
                <tbody>
                    @forelse ($mails as $mail)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4 font-medium">{{ $mail->sender }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $statusColor = match($mail->status) {
                                        'Diterima' => 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300',
                            ...
                            <td class="px-6 py-4 space-x-2 whitespace-nowrap">
                                ...
                                @endif
                                <a href="{{ route('admin.incoming-mails.edit', $mail) }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a> | 
                                <form action="{{ route('admin.incoming-mails.destroy', $mail) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus surat masuk ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center">Belum ada surat masuk.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
