<x-admin-layout>
    <div class="mb-6">
        <h2 class="font-semibold text-2xl text-gray-800 dark:text-gray-200 leading-tight">
            Edit Data Klien: {{ $client->name }}
        </h2>
    </div>

    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900 dark:text-gray-100">
            <form method="POST" action="{{ route('admin.client-data.update', $client) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                            <option value="Perorangan" {{ old('type', $client->type) == 'Perorangan' ? 'selected' : '' }}>Perorangan</option>
                            <option value="Perusahaan" {{ old('type', $client->type) == 'Perusahaan' ? 'selected' : '' }}>Perusahaan</option>
                            <option value="Institusi" {{ old('type', $client->type) == 'Institusi' ? 'selected' : '' }}>Institusi</option>
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="case_category" class="block mt-1 w-full" type="text" name="case_category" :value="old('case_category', $client->case_category)" />
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="4" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('notes', $client->notes) }}</textarea>
